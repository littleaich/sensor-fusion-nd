...
void proximity(const int id, const std::vector<std::vector<float>>& points,
				...
				std::vector<int> & cluster, KdTree * tree)
{
	...
	*/
	...
	std::vector<int> idsNeighborPoints ( tree->search(points[id], distanceTol) );	
	for (const auto & id : idsNeighborPoints) {
		if (processed[id]) continue;
		proximity(id, points, distanceTol, processed, cluster, tree);
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree * tree, const float distanceTol)
{
	...
	*/
	...
	for (int i{0}; i<points.size(); ++i) {
		...
		proximity(i, points, distanceTol, processed, cluster, tree);

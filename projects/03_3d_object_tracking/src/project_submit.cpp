...
int main(int argc, const char *argv[])
{
    ...
    // MAIN LOOP OVER ALL IMAGES 
  	...
  	const int types_ = 7;
  	string detectorTypeArr[] = {"shi-tomasi", "harris", "fast", "brisk", "orb", "akaze", "sift"};
    string descriptorTypeArr[] = {"brief", "brief", "brief", "brisk", "orb", "akaze", "sift"};
  	string matcherTypeMatchingArr[] = {"MAT_BF", "MAT_BF", "MAT_BF", "MAT_BF", "MAT_BF", "MAT_BF", "MAT_FLANN"};         
    string descriptorTypeMatchingArr[] = {"DES_BINARY", "DES_BINARY", "DES_BINARY", "DES_BINARY", "DES_BINARY", "DES_HOG", "DES_HOG"}; // DES_BINARY, DES_HOG
    string selectorTypeMatchingArr[] = {"SEL_NN", "SEL_NN", "SEL_NN", "SEL_NN", "SEL_NN", "SEL_NN", "SEL_KNN"};       // SEL_NN, SEL_KNN
  	...
  	outFile << "Frame,Detector,Descriptor,TTC-LiDAR(sec),TTC-Camera(sec)\n";
  	for (int ty{0}; ty<types_; ++ty) {
      	string detectorType = detectorTypeArr[ty];
      	string descriptorType = descriptorTypeArr[ty];
      	string matcherTypeMatching = matcherTypeMatchingArr[ty];
      	string descriptorTypeMatching = descriptorTypeMatchingArr[ty];
      	string selectorTypeMatching = selectorTypeMatchingArr[ty];
      
        int dataBufferSize = 2;       // no. of images which are held in memory (ring buffer) at the same time
    	std::deque<DataFrame> dataBuffer; // list of data frames which are held in memory at the same time
      
      	std::cout << "===================================================" << std::endl;
      	std::cout << "(Detector, Descriptor) = (" << detectorType << "," << descriptorType << ")" << std::endl;
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    computeTTCCamera((dataBuffer.end() - 2)->keypoints, (dataBuffer.end() - 1)->keypoints, currBB->kptMatches, sensorFrameRate, ttcCamera);
                        ...
                        std::cout << "(" << ttcLidar << "," << ttcCamera << ") seconds" << std::endl;
                  		...
                  		outFile << std::to_string(imgIndex) + "," + detectorType + ",";
                  		outFile << descriptorType + "," + std::to_string(ttcLidar) + ",";
                  		outFile << std::to_string(ttcCamera) + "\n";
                    #endif
